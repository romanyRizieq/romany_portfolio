...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    const user_email = formData.get("email")?.trim();
    const user_phone = formData.get("phone")?.trim();
    const user_reason = form.current.querySelector("#user_reason").selectedOptions[0]?.text;
    const user_message = formData.get("message")?.trim(); // Ensure message is captured correctly

    // Debugging log to check captured values
    console.log({
      user_name,
      user_email,
      user_phone,
    ...
    if (!user_name || !user_email || !user_phone || !user_reason || !user_message) {
      Swal.fire({
        icon: "error",
